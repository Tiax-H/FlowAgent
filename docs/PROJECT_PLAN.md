...
| —（版本不变） | 2026-08-29 | **Provider 网页端配置（加密存储）落地**：LLM Provider 除环境变量外支持在设置页增删改查——新增 Prisma `provider_configs` 表，apiKey 以 AES-256-GCM 加密落库（主密钥来自 `FLOWAGENT_SECRET_KEY`，未设置时网页端保存禁用并明示），密钥绝不回显（列表仅返回掩码尾 4 位）；同名时 env 与 DB 配置互斥（数据库不可创建与环境变量同名的 Provider）；`LlmAdapter` 改为按调用合并解析（env + DB），网页端保存后即时生效无需重启；新增 `POST/PATCH/DELETE /api/llm/providers`，响应与日志不含明文密钥。对齐 §安全「API key 只存于环境变量 / Provider 配置表（加密存储）」的双路径授权 |
